...
void AppManager::Run(){
    ...
    std::queue<int> intervals = _GenerateIntervals(0, 1000, _lineFrequency);

    deque<std::unique_ptr<Figure>> activeEntities;
    std::vector<std::unique_ptr<Figure>> to_add;
    ...
    while (true){
        ...
        while (!intervals.empty() && delta >= intervals.front()){
            activeEntities.emplace_back(std::make_unique<Line>(_lineLength, GetRandomRow(), _lineSpeed, GetNewLineColor(), _explosionFrequency));
        ...
        for (auto it = activeEntities.begin(); it < activeEntities.end();) {
            ...
            if ((*it)->HasExploded()  ) {
                (*it)->Explode();
                to_add.emplace_back(std::make_unique<Explosion>((*it)->GetCenterX(), (*it)->GetCenterY(), _minExplosionRadius, _maxExplosionRadius));
            }
        ...
        if (!to_add.empty()) {
            activeEntities.insert(activeEntities.end(),\
...
std::queue<int> AppManager::_GenerateIntervals(int start, int stop, int count){
    std::vector<int> temp(_lineFrequency);
    ...
    for (int v : temp) intervals.push(v);
    return intervals;
}
